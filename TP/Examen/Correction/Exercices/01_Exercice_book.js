...
db.books.insertMany([
  ...
  {
    title: "PosgreSQL",
    category_id: db.categories.findOne({ name: "SQL" })._id,
  },
  { title: "MySQL", category_id: db.categories.findOne({ name: "SQL" })._id },
  {
    title: "MongoDB",
    category_id: db.categories.findOne({ name: "NoSQL" })._id,
  },
]);
...
db.books.aggregate([
    ...
    },
    {
      $replaceRoot: {
        newRoot: {
          $mergeObjects: [{ $arrayElemAt: ["$fromCategoryId", 0] }, "$$ROOT"],
        },
      },
    },
    { $project: {  fromCategoryId: 0 } },
  ]);
...
// à l'aide de l'agrégation et de l'opérateur $lookup
...
db.books.insertOne({
  ...
  category_id: [programmation._id, sql._id],
});
db.books.insertOne({
  title: newBooks[1].title,
  category_id: [programmation._id, sql._id, noSql._id],
});

db.books.insertOne({
  title: newBooks[2].title,
  category_id: [programmation._id, sql._id, noSql._id],
});
